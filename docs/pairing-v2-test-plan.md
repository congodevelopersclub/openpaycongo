...
6. Server feature test issues fresh QR data, completes once with fresh client material, verifies encrypted response and `private, no-store`, then retries exact saved bytes and proves same encrypted `201` response without second pairing. Malformed, expired, unknown, altered, exhausted, or non-identical consumed attempts produce indistinguishable unavailable response. Invalid proofs consume the three-attempt per-intent budget under the row lock; expiry drains 100-row transactions until no due intent remains; and the native limiter admits ten requests per IP/minute. A protector, database, random-source, or uncertain-commit failure returns generic no-store `503`, preserving the exact retry path. Do not assert secret values in logs/errors.
