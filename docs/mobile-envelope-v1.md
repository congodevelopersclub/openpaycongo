...
Deposit request plaintext is UTF-8 JSON with exactly:
...
Final activation acknowledgement plaintext is UTF-8 JSON with exactly:

```json
{"version":1,"operation":"activation_acknowledgement","payload":{}}
```

Only native code creates this empty payload. A paired installation with no server-side activation acknowledgement may send this operation, but the server rejects every other operation using the indistinguishable unavailable response. This prevents a durable local installation from becoming usable before it has authenticated the encrypted server result.
...
For `activation_acknowledgement`, the server returns `201` even for a later-counter retry and the exact encrypted response plaintext is:

```json
{"outcome":"acknowledged"}
```

Android accepts this outcome only after status-bound AEAD verification, then atomically changes its protected record from pending to active. If the response is lost or cannot be authenticated, the record remains pending and the next retry reserves a new counter.
...
Pairing v2 owns the directional-key lifecycle in Android native code: it derives the keys with libsodium, holds them only in a process-scoped exchange while authenticating the completion response, then waits for activation to atomically promote the credential, installation identity, both directions, and the exact HTTPS origin derived from the signature-verified QR endpoint in one Keystore-backed vault record. Flutter receives only public completion request fields and the SAS. The native envelope path neither returns a directional key nor uses a bearer credential.

The native vault validates that origin again, returns it only as request-routing metadata alongside a newly sealed ciphertext, and will not infer an origin for an older active-generation record that did not store one. That upgrade path fails closed: re-pair the installation before it can use mobile envelopes. A version-4 origin-pinned record is different: the upgrade atomically rewrites it as version 5 pending acknowledgement while preserving its protected generation. It cannot send a normal operation until it completes the encrypted acknowledgement. It never falls back to the legacy credential-protected endpoint.
