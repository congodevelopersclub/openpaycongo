...
This document is normative with [ADR 004](adr-004-secure-device-enrollment.md). It intentionally specifies one operation, `deposit`; the Android adapter seals and decrypts in native code while Dart owns only bounded HTTPS routing and retry scheduling.
...
The native vault validates that origin again, returns it only as request-routing metadata alongside a newly sealed ciphertext, and will not infer an origin for an older active-generation record that did not store one. That upgrade path fails closed: re-pair the installation before it can use mobile envelopes. It never falls back to the legacy bearer-protected endpoint.
