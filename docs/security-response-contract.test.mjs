...
test('security response policy is private, accountable, and release-blocking', async () => {
  ...
  assert.match(security, /These steps do not create release, rollback, or emergency authority/i);
  assert.match(security, /repository administrator documents approval of an emergency-release authority/i);
  assert.match(security, /verifying ordinary release authority—or, if step 3 invoked the compromise\s+response/i);
  const rollbackCheckpoint = security.indexOf('For immediate containment, the release owner may execute a rollback before');
  ...
  const authorizationCheckpoint = security.indexOf('A patched release may proceed only under documented administrator release');
  ...
  assert.match(rollbackStep, /only under that documented administrator approval/i);
  assert.match(rollbackStep, /identifying the rollback artifact, recording why it\s+predates the affected\s+artifact and contains the incident/i);
  assert.match(rollbackStep, /verifying\s+ordinary release\s+authority—or restored authority if step 3 invoked the\s+compromise response/i);
  assert.match(rollbackStep, /rollback decision and user\s+communication in the\s+private record/i);
  assert.ok(advisoryCheckpoint >= 0, 'emergency procedure must create a private advisory after artifact evidence');
  assert.ok(authorizationCheckpoint >= 0, 'emergency procedure must condition a patched release on documented authority');
