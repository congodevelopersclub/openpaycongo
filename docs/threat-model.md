...
| Pairing QR interception, UI compromise, or key-material disclosure | ADR 004 constrains QR fields, cryptographic transcript, key destruction, no-store response policy, administrator comparison, audited operator revocation, and single-active re-pair replacement. | Compromised edge, administrator UI, or OAuth session is explicitly outside the current guarantee; multi-device recovery and any overlapping rollover protocol are future work. |
