...
The first native envelope read after this format change migrates an older separately encrypted credential record and directional-key record only when both authenticate. The shipped v1 directional record contains only keys, so its separately authenticated credential supplies the installation identity; a short-lived v2 directional record is accepted only when its embedded identity matches that credential. It serializes active-generation reads, writes, and migration under one process-wide lock. A missing, malformed, unauthenticated, or mismatched legacy record fails closed; it never guesses or partially migrates state.
