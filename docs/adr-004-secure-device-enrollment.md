...
The mobile runtime verifies the signed QR, completes the encrypted exchange, stores pending directional keys in Android Keystore, and displays the SAS while marking the device inactive. Current pairing v2 derives directional-key copies in Dart and transfers them through the pairing-key MethodChannel for this initial storage. That is an unresolved P1; this ADR does not claim directional keys never enter Dart until pairing key agreement and persistence are native.

After administrator confirmation, the user explicitly checks activation: the app retrieves only the opaque activation envelope over strict HTTPS without redirects, passes it to the Android native vault, decrypts it with the stored receiver key using official libsodium XChaCha20-Poly1305, and stores the credential in a no-backup Android Keystore-backed vault. This native activation path returns only a redacted outcome; it does not return the receiver key or issued credential. The issued token is never returned to the administrator session.

Laravel implements the server half of active mobile-envelope deposit transport; the exact contract is [mobile-envelope-v1.md](mobile-envelope-v1.md). Android #211 implements local native sealing: bounded deposit plaintext enters from Dart, a foreground/unlock-gated native vault reserves a Keystore-encrypted no-backup counter, reads its stored send key, and returns routing-safe envelope fields only. It does not implement delivery, response decryption, acknowledgement, revocation, rotation, or durable recovery. Do not claim plaintext never enters Dart: the outbound payload originates there. Do not claim a phone is active merely because the server sealed an activation envelope.
