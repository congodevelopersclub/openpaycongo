...
Laravel implements the server half of active mobile-envelope deposit transport; the exact contract is [mobile-envelope-v1.md](mobile-envelope-v1.md). Android #211 implements local native sealing: bounded deposit plaintext enters from Dart, a foreground/unlock-gated native vault persists the successor of a Keystore-encrypted no-backup counter before sealing, reads its stored send key, and returns routing-safe envelope fields only. The pure allocator tests restart, exhausted, corrupt, and write-failure outcomes; Android `AtomicFile` plus `fd.sync()` is the production persistence adapter. This does not prove power-loss behavior on every device/filesystem. Delivery, response decryption, acknowledgement, revocation, rotation, and durable recovery remain out of scope. Do not claim plaintext never enters Dart: the outbound payload originates there. Do not claim a phone is active merely because the server sealed an activation envelope.
