...
The mobile runtime verifies the signed QR, then sends its exact canonical HTTPS origin with the QR secret to the Android native pairing boundary, which pins that origin in the process-scoped pending exchange before generating the client keypair, deriving directional keys with libsodium, and sealing the secret. Activation accepts no origin from Dart; native code promotes only the origin pinned during that verified-QR begin together with the credential, installation identity, and both directions. The one-time QR secret crosses the bootstrap MethodChannel once; directional keys never cross it or enter Dart, a BLoC, state, logs, analytics, notifications, screenshots, or backups. Cancel, failure, replacement, or process restart before authenticated completion wipes that pending exchange and requires a fresh QR.

After native authentication of a valid completion response, Android persists one no-backup Keystore-encrypted confirmed exchange containing intent, pinned canonical HTTPS origin, both directional keys, and SAS. On a later unlocked foreground start, native restores it and returns only SAS plus native-originated public activation routing bytes to Dart; Dart still returns only the retrieved opaque activation envelope to native. Before active-generation promotion, native deletes and verifies removal of both confirmed-exchange `AtomicFile` base and backup records; deletion failure fails closed and requires re-pairing. Cancellation, a replacement QR, malformed recovery state, or terminal recovery error clears or rejects that exchange fail closed. This recovery does not resume a pre-completion request or extend its 15-second retry budget.

After administrator confirmation, the user explicitly checks activation: the app retrieves only the opaque activation envelope over strict HTTPS without redirects, passes it to the Android native pairing boundary, decrypts it with the pending receiver key using the official libsodium XChaCha20-Poly1305 implementation, and promotes the credential, installation identity, and both directional keys in one no-backup Android Keystore-backed record. The record is pending final acknowledgement, not active. This path returns only a redacted outcome; it does not return key material or the issued credential. The issued credential is never returned to the administrator session.

Older pairing records do not contain a QR-derived HTTPS origin. Native reads reject every such legacy record without promotion or inferred routing authority; recovery requires re-pairing from a newly signature-verified QR. Active-generation reads and writes remain serialized under one process-wide lock.

Laravel now implements the server half of active mobile-envelope deposit transport; the exact contract is [mobile-envelope-v1.md](mobile-envelope-v1.md). Android native code persists the successor of a Keystore-encrypted no-backup counter before sealing, reads its stored send key, receive key, installation identity, and pinned HTTPS origin from one atomically promoted generation, and returns only routing-safe envelope fields. Dart posts exactly that encrypted envelope with no bearer header, then returns bounded encrypted response metadata to native code for status-bound AEAD verification and outcome decryption. The pure allocator and response-contract tests cover restart, exhausted, invalid-state, write-failure, status/AAD, and malformed-outcome behavior; Android `AtomicFile` plus `fd.sync()` is the production persistence adapter. This does not prove power-loss behavior on every device/filesystem. Verified-operator revocation is audited without secrets, deletes the paired installation's Sanctum tokens, scrubs both directional keys and activation ciphertext, and rejects its old activation retrieval, mobile envelopes, and bearer access. A subsequent SAS-confirmed fresh QR atomically revokes every existing paired installation for the organization before issuing new pairwise keys and a new credential. This is single-active replacement: there is no overlapping rollover or multi-device protocol. Pre-completion durable retry recovery remains a follow-up slice. Do not claim plaintext never enters Dart: the outbound payload originates there. Do not claim a phone is active merely because the server sealed an activation envelope.
...
`ciphertext` uses the existing server-send / phone-receive `crypto_kx` key and XChaCha20-Poly1305-IETF. Its plaintext is a versioned native-only credential record containing the installation identity. It is never represented in Dart, fixture data, logs, telemetry, or UI state.
...
This is a distinct protocol domain, but uses no derived key, custom KDF, alternative AEAD, or duplicated crypto implementation. Laravel stores both protocol ciphertext and nonce with encrypted Eloquent casts; the raw credential exists only while the server creates and seals the envelope. The server does not log, serialize, return, or retain it in plaintext.

## Final activation acknowledgement

After the native activation record is durable, Android sends an encrypted `activation_acknowledgement` through the existing `POST /mobile/envelopes` route. The inner request has exactly version `1`, operation `activation_acknowledgement`, and an empty object payload. Dart handles only the route and outer ciphertext fields; it never receives plaintext, keys, or credentials.

The server authenticates and decrypts the directional envelope before recording `activation_acknowledged_at` in the same transaction as its consumed replay counter. It encrypts the exact `{"outcome":"acknowledged"}` response under the opposite directional key and returns HTTP `201` for every valid acknowledgement, including a later-counter retry. Native code verifies the response AAD, status, and exact outcome before atomically flipping its protected record to active. A lost, malformed, or unauthenticated response leaves the record pending and retries with a newly durable counter; it never silently becomes active.

Version-4 active-generation records already protected one complete credential, origin, and directional-key generation. On upgrade, Android atomically rewrites that protected record as version 5 pending acknowledgement and preserves its protected material. It does not infer acknowledgement or require re-pairing. Until the upgraded client sends the encrypted acknowledgement, the server rejects non-acknowledgement mobile-envelope operations with the same generic unavailable response. Older originless records remain unrecoverable and require a new signed QR pairing.

## Active mobile envelope contract

Laravel implements `POST /mobile/envelopes` protocol v1 for encrypted deposit submission and final activation acknowledgement. It uses a 24-byte random nonce plus XChaCha20-Poly1305-IETF under the pairing directional keys. No per-message signature: AEAD authenticates the sender holding its directional key. No cleartext business or PII payload reaches Laravel before envelope authentication/decryption. The exact request, response, AAD, counter, failure, and compatibility rules are in [mobile-envelope-v1.md](mobile-envelope-v1.md). PHP feature tests prove valid submission, replay, conflict, tampering, stale counters, installation retargeting, and acknowledgement gating; Android must independently prove compatible native encryption before any Dart transport adapter is released.
