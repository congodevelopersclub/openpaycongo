...
The normative protocol is [ADR 004](adr-004-secure-device-enrollment.md). Laravel pairing uses Eloquent models, migrations, Form Requests, Policies, transactional Actions, and encrypted casts. It issues QR v2, completes/replays one encrypted request to `pending_confirmation`, requires verified-administrator SAS confirmation, delivers and acknowledges activation, and accepts counter-bound active mobile envelopes. A verified operator can revoke a paired installation through Filament; the action is audited without secrets, deletes its Sanctum tokens, scrubs directional and activation material, and leaves old activation, envelope, and bearer access unavailable. A subsequent SAS-confirmed QR atomically replaces all existing paired installations for that organization before new pairwise keys and credentials are issued. This deliberately has no overlapping rollover or multi-device protocol; pre-completion retry recovery remains follow-up work.
