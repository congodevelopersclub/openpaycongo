...
const withHarness = async (run) => {
  ...
  for (const name of ['docker', 'bash', 'grep']) {
    ...
    const fake = name === 'docker'
      ? '#!/bin/sh\nstate() { [ -n "$1" ] && printf set || printf missing; }\nif [ "$1" = compose ] && [ "$2" = config ]; then\n  printf "docker compose config APP_KEY=%s DB_PASSWORD=%s LOOKUP_TOKEN=%s PASSPORT_PRIVATE_KEY_FILE=%s PASSPORT_PUBLIC_KEY_FILE=%s\\n" "$(state "$OPENPAY_APP_KEY")" "$(state "$OPENPAY_DB_PASSWORD")" "$(state "$DEPOSIT_LOOKUP_TOKEN_KEY")" "$(state "$OPENPAY_PASSPORT_PRIVATE_KEY_FILE")" "$(state "$OPENPAY_PASSPORT_PUBLIC_KEY_FILE")" >> "$FAST_FEEDBACK_LOG"\nelse\n  printf "docker %s\\n" "$*" >> "$FAST_FEEDBACK_LOG"\nfi\nif [ -n "$FAST_FEEDBACK_FAIL_MATCH" ] && printf "%s" "$*" | /bin/grep -Fq -- "$FAST_FEEDBACK_FAIL_MATCH"; then exit 97; fi\nif [ "$1" = compose ] && [ "$2" = config ]; then\n  case "$FAST_FEEDBACK_ACCEPT_MISSING" in APP_KEY) [ -z "$OPENPAY_APP_KEY" ] && exit 0 ;; DB_PASSWORD) [ -z "$OPENPAY_DB_PASSWORD" ] && exit 0 ;; LOOKUP_TOKEN) [ -z "$DEPOSIT_LOOKUP_TOKEN_KEY" ] && exit 0 ;; PASSPORT_PRIVATE_KEY_FILE) [ -z "$OPENPAY_PASSPORT_PRIVATE_KEY_FILE" ] && exit 0 ;; PASSPORT_PUBLIC_KEY_FILE) [ -z "$OPENPAY_PASSPORT_PUBLIC_KEY_FILE" ] && exit 0 ;; esac\n  [ -n "$OPENPAY_APP_KEY" ] && [ -n "$OPENPAY_DB_PASSWORD" ] && [ -n "$DEPOSIT_LOOKUP_TOKEN_KEY" ] && [ -n "$OPENPAY_PASSPORT_PRIVATE_KEY_FILE" ] && [ -n "$OPENPAY_PASSPORT_PUBLIC_KEY_FILE" ] || exit 1\nfi\ncase " $* " in *" psql "*) printf "%s\\n" "deposits_reverses_deposit_id_foreign:FOREIGN KEY" "deposits_reverses_deposit_id_unique:UNIQUE" ;; esac\n'
      : name === 'grep'
        ? '#!/bin/sh\nprintf "grep %s\\n" "$*" >> "$FAST_FEEDBACK_LOG"\nif [ -n "$FAST_FEEDBACK_FAIL_MATCH" ] && printf "%s" "$*" | /bin/grep -Fq -- "$FAST_FEEDBACK_FAIL_MATCH"; then exit 97; fi\nexec /bin/grep "$@"\n'
        : '#!/bin/sh\nprintf "bash %s\\n" "$*" >> "$FAST_FEEDBACK_LOG"\nif [ -n "$FAST_FEEDBACK_FAIL_MATCH" ] && printf "%s" "$*" | /bin/grep -Fq "$FAST_FEEDBACK_FAIL_MATCH"; then exit 97; fi\n';
...
test('runner dispatches actual tier commands and fails closed for unavailable tiers', async () => {
  await withHarness(async (invoke, calls, clearCalls, invokeWithFailure, invokeWithEnvironment) => {
    ...
    assert.deepEqual((await calls()).split('\n').filter((line) => line.startsWith('docker compose config')), [
      'docker compose config APP_KEY=set DB_PASSWORD=set LOOKUP_TOKEN=set PASSPORT_PRIVATE_KEY_FILE=set PASSPORT_PUBLIC_KEY_FILE=set',
      'docker compose config APP_KEY=missing DB_PASSWORD=set LOOKUP_TOKEN=set PASSPORT_PRIVATE_KEY_FILE=set PASSPORT_PUBLIC_KEY_FILE=set',
      'docker compose config APP_KEY=set DB_PASSWORD=missing LOOKUP_TOKEN=set PASSPORT_PRIVATE_KEY_FILE=set PASSPORT_PUBLIC_KEY_FILE=set',
      'docker compose config APP_KEY=set DB_PASSWORD=set LOOKUP_TOKEN=missing PASSPORT_PRIVATE_KEY_FILE=set PASSPORT_PUBLIC_KEY_FILE=set',
      'docker compose config APP_KEY=set DB_PASSWORD=set LOOKUP_TOKEN=set PASSPORT_PRIVATE_KEY_FILE=missing PASSPORT_PUBLIC_KEY_FILE=set',
      'docker compose config APP_KEY=set DB_PASSWORD=set LOOKUP_TOKEN=set PASSPORT_PRIVATE_KEY_FILE=set PASSPORT_PUBLIC_KEY_FILE=missing',
    ]);
    for (const acceptedMissing of ['APP_KEY', 'DB_PASSWORD', 'LOOKUP_TOKEN', 'PASSPORT_PRIVATE_KEY_FILE', 'PASSPORT_PUBLIC_KEY_FILE']) {
