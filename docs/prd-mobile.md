...
- The one-time QR secret crosses the bootstrap bridge once. Directional keys stay in a process-scoped native exchange
  until a response is authenticated and administrator-confirmed activation atomically promotes both directions, the
  installation identity, and the verified QR HTTPS origin in Keystore-backed secure storage; neither key enters Dart, a MethodChannel result,
