...
Enrollment expiry and proof attempts are fixed bounds. Exact completion retries return the stored encrypted result; changed retries, expired QR codes, exhausted attempts, and unknown identifiers share one unavailable response. Atomic completion clears the protected ephemeral key but retains non-secret replay metadata. Expiry/exhaustion clears ephemeral material; pending-confirmation timeout also clears the install root and SAS. Cleanup workers examine at most a configured page per run (protocol maximum 100 candidates), apply those transitions idempotently, and never use an unbounded scan. A crash after attempt reservation consumes that attempt; after losing saved mobile retry bytes, recovery requires discarding pending material and scanning a new QR rather than re-encrypting under a possibly reused nonce.

Outbound retry and circuit-breaker behavior is normative in [ADR 005](adr-005-bounded-outbound-resilience.md). It is not implemented in the pairing core because that core makes no outbound call; adding an unused generic retry library would create fake completeness.
