...
final class CanonicalAnalyticsProjection
{
    ...
    private function derive(array $events): array
    {
        ...
        foreach ($events as $event) {
            ...
            if (! isset($captures[$related])) {
                continue;
            }
            $capture = $captures[$related];
            if ($event['occurred_at'] < $capture['occurred_at'] || $event['currency'] !== $capture['currency'] || $event['provider'] !== $capture['provider']) {
                continue;
            }
            if ($event['kind'] === 'payment_voided' && $event['amount_minor'] === $capture['amount_minor']) {
                $voided[$related] = true;
            }
            if ($event['kind'] === 'payment_refunded') {
                $refunds[] = $event;
            }
            if ($event['kind'] === 'payment_reconciled' && (! isset($reconciled[$related]) || $event['received_at'] < $reconciled[$related]['received_at'])) {
                $reconciled[$related] = $event;
            }
        }
    ...
    private function window(array $state, string $from, string $to): array
    {
        ...
        $add = static function (array &$bucket, string $currency, string $amount, bool $refund): void {
            ...
            if ($refund) {
                $bucket[$currency]['refunds'] = $bucket[$currency]['refunds']->plus($amount);
                $bucket[$currency]['refund_count']++;
            } else {
                $bucket[$currency]['gross'] = $bucket[$currency]['gross']->plus($amount);
                $bucket[$currency]['payments']++;
            }
        };
        foreach ($state['captures'] as $id => $event) {
            if (! isset($state['voided'][$id]) && $event['occurred_at'] >= $from && $event['occurred_at'] < $to) {
                $add($currencies, $event['currency'], $event['amount_minor'], false);
                $providers[$event['provider']] ??= [];
                $add($providers[$event['provider']], $event['currency'], $event['amount_minor'], false);
            }
        }
        foreach ($state['refunds'] as $event) {
            if ($event['occurred_at'] >= $from && $event['occurred_at'] < $to) {
                $add($currencies, $event['currency'], $event['amount_minor'], true);
                $providers[$event['provider']] ??= [];
                $add($providers[$event['provider']], $event['currency'], $event['amount_minor'], true);
            }
        }
        ksort($currencies);
        ksort($providers);
    ...
    private function reconciliation(array $state, string $from, string $to, string $snapshotAt): array
    {
        $max = 0;
        $unreconciled = 0;
        foreach ($state['captures'] as $id => $capture) {
            if (! isset($state['voided'][$id]) && $capture['occurred_at'] >= $from && $capture['occurred_at'] < $to) {
                $end = $state['reconciled'][$id]['received_at'] ?? $snapshotAt;
                if (! isset($state['reconciled'][$id])) {
                    $unreconciled++;
                }
    ...
    private function cues(array $state, string $from, string $to, string $snapshotAt, array $sync): array
    {
        ...
        foreach ($state['events'] as $event) {
            if ($event['occurred_at'] >= $from && $event['occurred_at'] < $to && (new DateTimeImmutable($event['received_at']))->getTimestamp() - (new DateTimeImmutable($event['occurred_at']))->getTimestamp() > 3600) {
                $late++;
            }
        }
        $overdue = 0;
        foreach ($state['captures'] as $id => $capture) {
            if (! isset($state['voided'][$id]) && ! isset($state['reconciled'][$id]) && $capture['occurred_at'] >= $from && $capture['occurred_at'] < $to && (new DateTimeImmutable($snapshotAt))->getTimestamp() - (new DateTimeImmutable($capture['received_at']))->getTimestamp() > 86400) {
                $overdue++;
            }
        }
        $result = [];
        if ($late) {
            $result[] = ['kind' => 'late_arrival', 'count' => (string) $late, 'action' => 'review_offline_sync'];
        }
        if ($overdue) {
            $result[] = ['kind' => 'reconciliation_overdue', 'count' => (string) $overdue, 'action' => 'reconcile_provider'];
        }
        if ($sync['status'] === 'stale') {
            $result[] = ['kind' => 'stale_sync', 'count' => '1', 'action' => 'check_replica_sync'];
        }
        usort($result, static fn (array $left, array $right) => $left['kind'] <=> $right['kind']);

        return $result;
    }

    private function version(array $events): string
    {
        usort($events, static fn (array $left, array $right) => $left['event_id'] <=> $right['event_id']);

        return hash('sha256', implode('', array_map(static fn (array $event) => $event['event_id'].hex2bin($event['payload_digest']), $events)));
    }

    private function buckets(array $query): array
    {
        $zone = new DateTimeZone($query['time_zone']);
        $from = new DateTimeImmutable($query['from']);
        $local = $from->setTimezone($zone);
        $next = $query['interval'] === 'hour' ? $from->add(new DateInterval('PT1H')) : (new DateTimeImmutable($local->format('Y-m-d').' 00:00:00', $zone))->add(new DateInterval('P1D'))->setTimezone(new DateTimeZone('UTC'));

        return [[$query['from'], min($next->format('Y-m-d\\TH:i:s\\Z'), $query['to'])]];
    }
