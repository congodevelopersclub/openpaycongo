...
func TestOpenAppliesChecksummedMigrationIdempotently(t *testing.T) {
	...
	if store.MigrationRevision() != "0004" {
...
func TestAcknowledgeAdvancesOnlyContiguousTenantAcceptance(t *testing.T) {
	store, err := Open(filepath.Join(t.TempDir(), "analytics.db"))
	if err != nil {
		t.Fatal(err)
	}
	...
	var firstCursor, secondCursor uint64
	if err := store.db.QueryRow("SELECT accepted_sequence FROM sales_analytics_events WHERE event_id=?", first.ID).Scan(&firstCursor); err != nil {
		t.Fatal(err)
	}
	if err := store.db.QueryRow("SELECT accepted_sequence FROM sales_analytics_events WHERE event_id=?", second.ID).Scan(&secondCursor); err != nil {
		t.Fatal(err)
	}
	if err := store.Acknowledge(context.Background(), first.TenantID, "replica-test", secondCursor); !errors.Is(err, analytics.ErrEventConflict) {
		t.Fatalf("gap acknowledgement: %v", err)
	}
	if err := store.Acknowledge(context.Background(), first.TenantID, "replica-test", firstCursor); err != nil {
		t.Fatal(err)
	}
	if err := store.Acknowledge(context.Background(), first.TenantID, "replica-test", firstCursor); err != nil {
		t.Fatalf("exact acknowledgement replay: %v", err)
	}
	if err := store.Acknowledge(context.Background(), first.TenantID, "replica-test", secondCursor); err != nil {
		t.Fatal(err)
	}
}
